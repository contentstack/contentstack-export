...
var contentTypesFolderPath = path.resolve(sanitizePath(config.data), sanitizePath(contentTypeConfig.dirName));
...
ExportContentTypes.prototype = {
  ...
  writeContentTypes: function () {
    ...
    return new Promise(function (resolve) {
      helper.writeFile(path.join(sanitizePath(contentTypesFolderPath), 'schema.json'), self.content_types);
      self.content_types.forEach(function (content_type) {
        helper.writeFile(path.join(sanitizePath(contentTypesFolderPath), sanitizePath(content_type.uid) + '.json'),
          content_type);
      });
