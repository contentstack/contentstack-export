...
module.exports = function (config) {
  return new Promise(function (resolve, reject) {
    if(config.email && config.password) {
      ...
      log.success(chalk.blue('Content types, entries, assets, labels, global fields, extensions modules will be exported'))
      log.success(chalk.red('Email, password, or management token is not set in the config, cannot export Webhook and label modules'))
      config.headers = {
        api_key: config.source_stack,
        access_token: config.access_token,
        'X-User-Agent': 'contentstack-export/v' + pkg.version
