...
var assetsFolderPath = path.resolve(sanitizePath(config.data), sanitizePath(assetConfig.dirName));
var assetContentsFile = path.resolve(sanitizePath(assetsFolderPath), 'assets.json');
var folderJSONPath = path.resolve(sanitizePath(assetsFolderPath), 'folders.json');
...
ExportAssets.prototype = {
  ...
  getVersionedAssetJSON: function (uid, version, bucket) {
    ...
    return new Promise(function (resolve, reject) {
      if (version <= 0) {
        var assetVersionInfoFile = path.resolve(sanitizePath(assetsFolderPath), sanitizePath(uid), '_contentstack_' + sanitizePath(uid) + '.json');
  ...
  downloadAsset: function (asset) {
    var self = this;
    return new Promise(function (resolve, reject) {
      var assetFolderPath = path.resolve(sanitizePath(assetsFolderPath), sanitizePath(asset.uid));
      var assetFilePath = path.resolve(sanitizePath(assetFolderPath), sanitizePath(asset.filename));
      ...
      self.assetStream = {
        url: assetConfig.secured
          ? `${asset.url}?authtoken=${config.authtoken}`
          : asset.url,
      };
