...
var entryFolderPath = path.resolve(sanitizePath(config.data), sanitizePath(config.modules.entries.dirName));

var localesFilePath = path.resolve(sanitizePath(config.data), sanitizePath(config.modules.locales.dirName), sanitizePath(config.modules.locales
  .fileName));
var schemaFilePath = path.resolve(sanitizePath(config.data), sanitizePath(config.modules.content_types.dirName), 'schema.json');
...
exportEntries.prototype.getEntry = function (apiDetails) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    return request(requestObject).then(function (response) {
      var entryPath = path.join(sanitizePath(entryFolderPath), sanitizePath(apiDetails.locale), sanitizePath(apiDetails.content_type),
        sanitizePath(response.body.entry.uid));
      mkdirp.sync(entryPath);
      helper.writeFile(path.join(sanitizePath(entryPath), 'version-' + sanitizePath(response.body.entry._version) +
...
exportEntries.prototype.getEntries = function (apiDetails) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    return request(requestObject).then(function (response) {
      ...
      if (!fs.existsSync(path.join(sanitizePath(entryFolderPath), sanitizePath(apiDetails.content_type)))) {
        mkdirp.sync(path.join(sanitizePath(entryFolderPath), sanitizePath(apiDetails.content_type)));
      }
      var entriesFilePath = path.join(sanitizePath(entryFolderPath), sanitizePath(apiDetails.content_type), sanitizePath(apiDetails.locale) + '.json');
      ...
      if (typeof config.versioning === 'boolean' && config.versioning) {
        for (var locale in locales) {
          ...
          content_types.forEach(function (content_type) {
            ...
            var versionedEntryFolderPath = path.join(sanitizePath(entryFolderPath), sanitizePath(locales[locale].code),
              sanitizePath(content_type.uid));
